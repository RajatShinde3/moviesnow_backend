...
class MediaAsset(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["season_id", "title_id"], ["seasons.id", "seasons.title_id"],
            ondelete="CASCADE", name="fk_media_assets_season_title_consistent",
        ),
        ForeignKeyConstraint(
            ["episode_id", "season_id", "title_id"],
            ["episodes.id", "episodes.season_id", "episodes.title_id"],
            ondelete="CASCADE", name="fk_media_assets_episode_season_title_consistent",
        ),
    )

    # ─────────────── Kind / Language ───────────────
    kind = Column(SAEnum(MediaAssetKind, name="media_asset_kind"), nullable=False, index=True,
    ...
    __table_args__ = __table_args__ + (
        ...
        CheckConstraint("(title_id IS NOT NULL) OR (season_id IS NOT NULL) OR (episode_id IS NOT NULL)",
                        name="ck_media_assets_has_some_scope"),
        CheckConstraint("(season_id IS NULL) OR (title_id IS NOT NULL)",
                        name="ck_media_assets_season_requires_title"),
        CheckConstraint("(episode_id IS NULL) OR (season_id IS NOT NULL AND title_id IS NOT NULL)",
        ...
        CheckConstraint("(language IS NULL) OR (length(btrim(language)) > 0)", name="ck_media_assets_language_not_blank"),
        # Deterministic featured asset per (scope, kind, language)
        Index("uq_media_assets_primary_per_scope",
              "title_id", "season_id", "episode_id", "kind", "language",
    ...
    # ─────────────── Relationships ───────────────
    title = relationship("Title", back_populates="media_assets", lazy="selectin", passive_deletes=True)
    season = relationship("Season", back_populates="media_assets", lazy="selectin", passive_deletes=True)
    episode = relationship("Episode", back_populates="media_assets", lazy="selectin", passive_deletes=True)

    uploaded_by = relationship("User", lazy="selectin", passive_deletes=True)
