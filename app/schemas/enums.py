...
Design notes
------------
â€¢ All enums subclass `str, PyEnum` for JSON-friendly serialization.
â€¢ VALUE STRINGS are **stable** once deployed (DB enums depend on them).
â€¢ Grouped by domain for clarity; keep `__all__` in sync when adding new enums.
"""
...
class StreamProtocol(str, PyEnum):
    ...
    DASH = "DASH"  # MPEG-DASH
    MP4 = "MP4"    # Progressive (direct file) â€” also used for downloads
...
class MediaAssetKind(str, PyEnum):
    ...
    CAPTION = "caption"
    AUDIO = "audio"
    # Extended kinds for delivery/storage policy
    ORIGINAL = "original"
    DOWNLOAD = "download"
    HLS = "hls"
    ARTWORK = "artwork"
...
class DrmScheme(str, PyEnum):
    """
    MoviesNow â€” DRM scheme (session-level)
