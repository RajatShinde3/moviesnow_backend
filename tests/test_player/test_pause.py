# tests/test_player/test_pause.py
import importlib
...
class HeartbeatInput(BaseModel):
    # Typical pause payload bits; keep optional so {} is valid
...
def _mk_app(monkeypatch, *, repo: Optional[FakeRepo] = None):
    ...
    /api/v1/player/sessions, and override everything pause() touches.
    """
...
def test_pause_happy_202_empty_body_and_headers(monkeypatch):
    ...
    resp = client.post(f"/api/v1/player/sessions/{sess_id}/pause", json=_body(), headers=headers)
...
def test_pause_excludes_none_fields_and_sets_flags(monkeypatch):
    app, client, mod, repo, calls = _mk_app(monkeypatch)

    # Only send position; others None → excluded by exclude_none
    minimal = {"position_sec": 10.0, "reason": None, "bitrate_kbps": None}
    resp = client.post("/api/v1/player/sessions/sess_1/pause", json=minimal)
    assert resp.status_code == 202

    payload = repo.append_calls[-1][2]
    assert payload == {"position_sec": 10.0, "event": "pause", "playing": False}


def test_pause_404_when_repo_returns_false(monkeypatch):
    r = FakeRepo(return_ok=False)
    app, client, mod, repo, calls = _mk_app(monkeypatch, repo=r)

    resp = client.post("/api/v1/player/sessions/sess_ABC12345/pause", json=_body())
...
def test_pause_500_when_repo_raises(monkeypatch):
...
def test_pause_multiple_calls_increment_deps(monkeypatch):
    app, client, mod, repo, calls = _mk_app(monkeypatch)

    _ = client.post("/api/v1/player/sessions/sess_1/pause", json=_body())
    _ = client.post("/api/v1/player/sessions/sess_2/pause", json=_body())
