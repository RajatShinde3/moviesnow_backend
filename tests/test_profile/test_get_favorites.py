# tests/test_user/test_get_favorites.py
...
class DummyFavorites:
    """Schema-agnostic stand-in for Favorites(items=[...])."""
...
def _mk_app(
    ...
    *,
    ...
    monkeypatch.setattr(mod, "Favorites", DummyFavorites, raising=True)
...
def test_favorites_happy_path_no_store_and_logging(monkeypatch):
    repo = FakeUserRepo(items=["tt1", "tt9", "tt2"])
    _app, client, _mod, log_calls = _mk_app(monkeypatch, repo=repo, record_log=True)

    r = client.get(_url())
    assert r.status_code == 200
    body = r.json()
    assert body == {"items": ["tt1", "tt9", "tt2"]}
    ...
    # logging
    assert any(
        c["action"] == "FAVORITES_GET"
        and c["user_id"] == "u-123"
        and c["meta"].get("count") == 3
        for c in log_calls
    )
...
def test_favorites_order_is_preserved(monkeypatch):
...
def test_favorites_empty_list(monkeypatch):
...
def test_favorites_auth_dependency_unauthorized(monkeypatch):
...
def test_favorites_repo_exception_yields_500(monkeypatch):
...
def test_favorites_api_key_enforcement_plain(monkeypatch):
    key = "sekret123"
    repo = FakeUserRepo(items=["tt1"])
...
def test_favorites_api_key_enforcement_hashed(monkeypatch):
    key = "super-secret"
    repo = FakeUserRepo(items=["tt1"])
