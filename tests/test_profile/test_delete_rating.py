# tests/test_user/test_delete_rating.py
...
class FakeUserRepo:
    ...
    def delete_rating(self, user_id: str, title_id: str):
        self.calls += 1
...
def test_delete_rating_happy_path_persists_and_logs(monkeypatch):
    user_repo = FakeUserRepo()
...
def test_delete_rating_applies_sanitize_and_forwards(monkeypatch):
    user_repo = FakeUserRepo()
...
def test_delete_rating_user_repo_exception_returns_500(monkeypatch):
...
def test_delete_rating_auth_dependency_unauthorized(monkeypatch):
    user_repo = FakeUserRepo()
...
def test_delete_rating_api_key_enforcement_plain(monkeypatch):
    key = "sekret123"
    user_repo = FakeUserRepo()
...
def test_delete_rating_api_key_enforcement_hashed(monkeypatch):
    key = "super-secret"
    user_repo = FakeUserRepo()
