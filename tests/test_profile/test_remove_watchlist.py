# tests/test_user/test_remove_watchlist.py
...
class FakeUserRepo:
    ...
    def remove_watchlist(self, user_id: str, title_id: str):
        self.calls += 1
...
def test_remove_watchlist_happy_path_persists_and_logs(monkeypatch):
    user_repo = FakeUserRepo()
...
def test_remove_watchlist_applies_sanitize_and_forwards(monkeypatch):
    user_repo = FakeUserRepo()
    _app, client, mod, _ = _mk_app(
        monkeypatch, user_repo=user_repo, sanitize_passthrough=False, record_log=False
    )
...
def test_remove_watchlist_user_repo_exception_returns_500(monkeypatch):
...
def test_remove_watchlist_auth_dependency_unauthorized(monkeypatch):
    user_repo = FakeUserRepo()
...
def test_remove_watchlist_api_key_enforcement_plain(monkeypatch):
    key = "sekret123"
    user_repo = FakeUserRepo()
...
def test_remove_watchlist_api_key_enforcement_hashed(monkeypatch):
    key = "super-secret"
    user_repo = FakeUserRepo()
