# tests/test_user/test_add_favorite.py
...
class FakeUserRepo:
    ...
    def add_favorite(self, user_id: str, title_id: str):
        self.calls += 1
...
def test_add_favorite_happy_path_persists_and_logs(monkeypatch):
    user_repo = FakeUserRepo()
    ...
    assert client.post(_url("ttX")).headers.get("Cache-Control") is None


def test_add_favorite_applies_sanitize_and_forwards(monkeypatch):
    user_repo = FakeUserRepo()
...
def test_add_favorite_user_repo_exception_returns_500(monkeypatch):
...
def test_add_favorite_auth_dependency_unauthorized(monkeypatch):
    user_repo = FakeUserRepo()
...
def test_add_favorite_api_key_enforcement_plain(monkeypatch):
    key = "sekret123"
    user_repo = FakeUserRepo()
...
def test_add_favorite_api_key_enforcement_hashed(monkeypatch):
    key = "super-secret"
    user_repo = FakeUserRepo()
