# tests/test_user/test_set_rating.py
...
class FakeUserRepo:
    ...
    def set_rating(self, user_id: str, title_id: str, rating: float):
        self.calls += 1
...
def test_set_rating_happy_path_persists_and_logs(monkeypatch):
    ...
    assert any(
        ...
        and c["meta"].get("rating") == 8.5
        for c in log_calls
    )

    # 204 responses shouldn't include cache headers by default
    assert r.headers.get("Cache-Control") is None


def test_set_rating_applies_sanitize_and_forwards(monkeypatch):
    user_repo = FakeUserRepo()
    ...
    r = client.post(_url("WeIRD-slug_123"), json={"rating": 9})
    assert r.status_code == 204
    assert user_repo.last_tid == "SANITIZED"


def test_set_rating_validation_422_for_missing_or_out_of_range(monkeypatch):
    user_repo = FakeUserRepo()
    _app, client, _mod, _ = _mk_app(monkeypatch, user_repo=user_repo)

    # body required
    assert client.post(_url("tt1")).status_code == 422

    # wrong type
    assert client.post(_url("tt1"), json={"rating": "nope"}).status_code == 422

    # out of range (assuming schema enforces 0..10 inclusive)
    assert client.post(_url("tt1"), json={"rating": -1}).status_code == 422
    assert client.post(_url("tt1"), json={"rating": 10.1}).status_code == 422


def test_set_rating_user_repo_exception_returns_500(monkeypatch):
    user_repo = FakeUserRepo(raise_exc=True)
    _app, client, _mod, _ = _mk_app(monkeypatch, user_repo=user_repo)

    r = client.post(_url("ttZ"), json={"rating": 7})
    assert r.status_code == 500


def test_set_rating_auth_dependency_unauthorized(monkeypatch):
    user_repo = FakeUserRepo()
    ...
    r = client.post(_url("tt1"), json={"rating": 6})
    assert r.status_code == 401
    assert user_repo.calls == 0


def test_set_rating_api_key_enforcement_plain(monkeypatch):
    key = "sekret123"
    user_repo = FakeUserRepo()
    _app, client, _mod, _ = _mk_app(monkeypatch, user_repo=user_repo, enforce_key=key, hashed=False)

    r1 = client.post(_url("tt1"), json={"rating": 5})
    assert r1.status_code == 401

    r2 = client.post(_url("tt1"), json={"rating": 5}, headers={"X-API-Key": key})
    assert r2.status_code == 204

    r3 = client.post(_url("tt1") + f"?api_key={key}", json={"rating": 5})
    assert r3.status_code == 204


def test_set_rating_api_key_enforcement_hashed(monkeypatch):
    key = "super-secret"
    user_repo = FakeUserRepo()
