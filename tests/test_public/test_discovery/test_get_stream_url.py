# tests/test_public/test_discovery/test_get_stream_url.py
...
class FakeRepo:
    ...
    def get_stream_resource_path(self, tid):
        self.last_tid = tid
        if self.raise_exc:
            raise RuntimeError("boom")
        # pretend origin path provided by repo
        return f"origin/{tid}/master.m3u8"
...
class SignerRecorder:
    ...
    def __call__(self, *, resource_path, quality, expires_in, purpose):
        ...
        # build a deterministic URL for assertions
...
def _url(tid: str, quality: str, **params):
    base = f"/api/v1/stream/{tid}/{quality}"
    if not params:
        return base
...
def test_stream_url_happy_path_uses_repo_path_and_no_store_headers(monkeypatch):
    repo = FakeRepo()
    signer = SignerRecorder()
    ...
    call = signer.calls[0]
    assert call["resource_path"] == "origin/toy-story/master.m3u8"
    # QualityEnum coerces from path; ensure value is correct
    assert getattr(call["quality"], "value", str(call["quality"])) == "1080p"
    assert call["expires_in"] == 3600
    assert call["purpose"] == "stream"
...
def test_stream_url_defaults_to_fallback_resource_path_when_method_missing(monkeypatch):
    signer = SignerRecorder()
    # object() has no get_stream_resource_path
    _app, client, mod, signer = _mk_app(
    ...
    assert r.status_code == 200
    call = signer.calls[0]
    assert call["resource_path"] == "stream/tt123"  # fallback path
    assert getattr(call["quality"], "value", str(call["quality"])) == "720p"
    assert call["expires_in"] == 1200


def test_stream_url_falls_back_when_repo_raises(monkeypatch):
    ...
    assert r.status_code == 200
    call = signer.calls[0]
    assert call["resource_path"] == "stream/slug"  # fallback on exception
    assert getattr(call["quality"], "value", str(call["quality"])) == "480p"


def test_stream_url_quality_validation_400_for_unsupported(monkeypatch):
    repo = FakeRepo()
    signer = SignerRecorder()
    _app, client, mod, signer = _mk_app(
        monkeypatch, repo_provider=lambda: repo, signer=signer
    )
    # QualityEnum likely contains 2160p; route restricts to {480p,720p,1080p} and should raise 400
    r = client.get(_url("slug", "2160p"))
    assert r.status_code == 400
    assert r.json()["detail"] == "Unsupported quality; allowed: 480p, 720p, 1080p"
    # signer should not be called
    assert len(signer.calls) == 0


def test_stream_url_expires_in_query_bounds_422(monkeypatch):
    repo = FakeRepo()
    signer = SignerRecorder()
    _app, client, mod, signer = _mk_app(
        monkeypatch, repo_provider=lambda: repo, signer=signer
...
def test_stream_url_enforces_public_api_key_plain(monkeypatch):
    key = "sekret123"
    repo = FakeRepo()
    signer = SignerRecorder()
    _app, client, mod, signer = _mk_app(
...
def test_stream_url_enforces_public_api_key_hashed(monkeypatch):
    key = "super-secret"
    repo = FakeRepo()
    signer = SignerRecorder()
    _app, client, mod, signer = _mk_app(
...
def test_stream_url_applies_sanitize_title_id(monkeypatch):
    repo = FakeRepo()
    signer = SignerRecorder()
    _app, client, mod, signer = _mk_app(
        monkeypatch,
    ...
    assert signer.calls[0]["resource_path"] == "origin/SANITIZED/master.m3u8"


def test_stream_url_signer_exception_returns_500(monkeypatch):
    repo = FakeRepo()
