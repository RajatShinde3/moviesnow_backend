# tests/test_admin/test_titles/test_publish_title.py
...
class TitleRow:
    def __init__(self, *, is_published: bool = False):
        self.id = uuid.uuid4()
...
class FakeDB:
    ...
    async def execute(self, stmt):  # noqa: ARG002 - we ignore the statement
...
def _mk_app(
    ...
    *,
    ...
    # Provide a harmless "Title" class reference so attribute access in query code is OK.
    # We won't let SQLAlchemy touch it; we also stub out "select" below.
    TitleCls = type("Title", (), {"id": object()})
    monkeypatch.setattr(mod, "Title", TitleCls, raising=False)

    # Replace sqlalchemy.select with a lightweight shim that supports .where().with_for_update()
...
def test_publish_title_happy_path_sets_flag_and_no_store_and_audits(monkeypatch):
    row = TitleRow(is_published=False)
    app, client, mod, db, user, calls, locks, audit_calls = _mk_app(monkeypatch, row=row)

    resp = client.post(f"/api/v1/admin/titles/{row.id}/publish")
    ...
    assert locks.lock_calls and locks.lock_calls[-1][0] == f"lock:admin_titles:publish:{row.id}"
    ...
    # Audit recorded
    assert audit_calls and audit_calls[-1][0] == "TITLES_PUBLISH"
...
def test_publish_title_idempotent_when_already_published(monkeypatch):
    row = TitleRow(is_published=True)
    app, client, mod, db, user, calls, locks, audit_calls = _mk_app(monkeypatch, row=row)

    resp = client.post(f"/api/v1/admin/titles/{row.id}/publish")
    assert resp.status_code == 200
    assert resp.json() == {"message": "Already published"}
...
def test_publish_title_404_when_missing_sets_no_store(monkeypatch):
    app, client, mod, db, user, calls, locks, audit_calls = _mk_app(monkeypatch, row=None)
    tid = uuid.uuid4()

    resp = client.post(f"/api/v1/admin/titles/{tid}/publish")
    ...
    # Lock acquired
    assert locks.lock_calls and locks.lock_calls[-1][0] == f"lock:admin_titles:publish:{tid}"
...
def test_publish_title_audit_error_is_swallowed(monkeypatch):
    row = TitleRow(is_published=False)
...
def test_publish_title_calls_security_checks(monkeypatch):
    row = TitleRow(is_published=False)
    app, client, mod, db, user, calls, locks, audit_calls = _mk_app(monkeypatch, row=row)
