# tests/test_admin/test_staff/test_remove_admin.py
...
def _mk_app(
    ...
    *,
    ...
    path = "/api/v1/admin/staff/{user_id}/remove-admin"
...
def test_remove_admin_happy_path_demotes_and_sets_snapshot(monkeypatch):
    # Import module to access enum BEFORE building db_row
    mod = importlib.import_module("app.api.v1.routers.admin.staff")
    ...
    resp = client.post(
        f"/api/v1/admin/staff/{user_id}/remove-admin",
    ...
    body = resp.json()
    assert body == {"message": "Removed ADMIN", "user": {"email": "u@example.com"}, "role": "USER"}
...
def test_remove_admin_target_not_admin_400(monkeypatch):
    mod = importlib.import_module("app.api.v1.routers.admin.staff")
    db_row = RowUser(email="x@example.com", role=mod.UserRole.USER)

    app, client, mod2, db, r, st, _ = _mk_app(monkeypatch, db_row=db_row)

    resp = client.post(
        f"/api/v1/admin/staff/{uuid.uuid4()}/remove-admin",
        json={"reauth_token": "rt"},
    )
    assert resp.status_code == 400
    assert "Target is not ADMIN" in resp.text


def test_remove_admin_requires_reauth_token(monkeypatch):
    mod = importlib.import_module("app.api.v1.routers.admin.staff")
    ...
    resp = client.post(f"/api/v1/admin/staff/{user_id}/remove-admin", json={})
    assert resp.status_code == 400
    assert "reauth_token required" in resp.text


def test_remove_admin_cannot_change_own_role(monkeypatch):
    mod = importlib.import_module("app.api.v1.routers.admin.staff")
    ...
    resp = client.post(
        f"/api/v1/admin/staff/{me.id}/remove-admin",
        json={"reauth_token": "rt"},
    )
    assert resp.status_code == 400
...
def test_remove_admin_404_when_user_not_found(monkeypatch):
...
def test_remove_admin_idempotency_replay_skips_db(monkeypatch):
    snap = {"message": "Removed ADMIN", "user": {"email": "snap@example.com"}, "role": "USER"}

    app, client, mod2, db, r, st, _ = _mk_app(
        monkeypatch,
        db_row=None,  # not used on replay
        idem_snapshot=snap,
    )
    user_id = uuid.uuid4()
...
def test_remove_admin_idempotency_set_failure_is_swallowed(monkeypatch):
    mod = importlib.import_module("app.api.v1.routers.admin.staff")
